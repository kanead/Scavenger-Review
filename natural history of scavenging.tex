...
We use these components to highlight the morphologies and environments that would have been conducive to scavenging over geological time by focussing on the dominant vertebrate groups of the land, sea and air. 
...
It seems probable that extinct species used soaring as a means for scavenging. 
...
In fact, the total cost of movement (again in J kg\textsuperscript{-1} m\textsuperscript{-1}) in (salmonid) fish is lower than either running or flying where it scales according to 2.15 $\times$ body mass (kg) \textsuperscript{-0.25} \citep{williams1999evolution} with only \textit{soaring} flight likely to surpass it. 
...
There is evidence too of scavening in extinct species, where shark teeth have been found in the remains of dinosaurs, mosasaurs and Pliocene (5.3 to 3.6 Mya) mysticete whales \citep{schwimmer1997scavenging,ehret2009caught}. 
...
For instance, early whales such as \textit{Basilosaurus} (38 to 36.5 Mya) seem to fit into the same niche as killer whales (\textit{Orcinus orca}) and we have some evidence for scavenging in both \citep{fahlke2012bite,Whitehead415}.
...
These insights allow us to compare extant terrestrial species to their prehistoric forebears given the dominance of mammalian carnviores since the Eocene (56-33.9 Million years ago; Mya) where the order split into the Caniforma and Feliforma \citep{van1987skeletal}.
...
So being able to overcome competitors, maximse the nutrient gain from the remnants, and survive long enough between meals are all essential parts of carcass handling time. 
...
In the ability to eat bone scavengers have arrived at a way to feed on a resource that is typcially too hard for many predators to process. 
...
The use of tools and the cooperative nature of hominins meant they could likely get a substantial part of their energetic requirements through scavenging depending on their environment \citep{moleon2014humans}. %TG: I've removed the intelligence bit from the sentence.
...
\cite{rayfield2001cranial} note this as being powerful enough to break open skeletal material \citep{rayfield2001cranial}.
