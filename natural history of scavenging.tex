...
In the simplest case, this rate can be thought of in terms of a gas diffusion model where the movement of two agents depends only on their relative speed. As carcasses are stationary, the relative speed between a scavenger and carrion is dependent on the movement of the scavenger. 
Vertebrates do more than simply bump into resources like gas molecules though, because they can actively detect resources through their sensory abilities. 
Scavenging will also depend on search rates, which are dependent on both species physiology and the dimensionality of the environment (\citep{pawar2012dimensionality}).
...
By depending on thermal air flows these species can forage over enormous ranges \citep{spiegel2013factors}. 
...
Indeed, scavenging in the diverse pterosaurs has been hypothesied many times before \citep{witton2008reappraisal}. 
...
Similarly we would expect that extinct species would also gain in a similar fashion depending on the efficiency of flight. 
...
Similarly someithing about the effecieny of pterosuars flight. 
The importance of efficient flying over large areas may explain the lack of scavenging behavior in bats as they are generally noctural, a time when they would receive no aid from convective air currents \citep{norberg2012vertebrate}. 
...
That said, \textit{Necromatis} (``death-eater''), a large bat from the middle to late Eocene (56 to 33.9 Mya) had a robust cranio-mandibular morphology, and is a likely candidate for scavenging behavior amongst the x amount of known bat species \citep{Weithofer_Necromantis_1887,Hand_Necromantis_2012}.
...
Many shark species show very large ranges (e.g. great white sharks (\textit{Carcharodon carcharias})) and have the lowest cost of swimming (I know the Greenland shark is the slowest swimmer). 
Faculative scavenging is seen in many marine groups including species of extant sharks such as White sharks, that are known to feed on whale carcasses \citep{fallows2013white}, Greenland sharks feeding on seals \citep{watanabe2012slowest}, and sixgill sharks \citep{anderson2016impact}.
There is evidence too of scavening in extinct species, where shark teeth have been found in the remains of dinosaurs, mososaurs and 4-million-year-old mysticete whales \citep{schwimmer1997scavenging,ehret2009caught}. 
...
The idea that the evolutionary transition in posture from the sprawling gait of reptiles to the erect posture of mammals conferred a huge advantage to the latter is often repeated e.g. benefits in terms of speed, efficiency, muscle effort and maneuverability \citep{sullivan2015posture}. 
...
Snakes are a bit different are more effecent at moving I think - lots of scavanging. 
Both these groups are more likely to depend on low energetic costs in order to componsate for low ecnounter rates duee to thier inabilty to move. 
...
The evolution of bipedal movment freed up the forelimbs for other purposes e.g. wings, tool-use etc. 
